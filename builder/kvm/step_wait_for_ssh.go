...
func (s *stepWaitForSSH) dhcpLeaseLookup(kvmPath string) (GuestIPFinder, error) {
...
*/
	macAddress := ""
/*
	if macAddress, ok = kvmData["ethernet0.address"]; !ok || macAddress == "" {
		if macAddress, ok = kvmData["ethernet0.generatedAddress"]; !ok || macAddress == "" {
			return nil, errors.New("couldn't find MAC address in KVM")
		}
	}
*/
...
/*
		...
		ipLookup, err := s.dhcpLeaseLookup(kvmPath)
		if err != nil {
