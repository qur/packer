...
func TestBuilderPrepare_KVMData(t *testing.T) {
	var b Builder
	config := testConfig()

	config["kvm_data"] = map[interface{}]interface{}{
		"one": "foo",
		"two": "bar",
	}
	...
	if len(b.config.KVMData) != 2 {
		t.Fatal("should have two items in KVMData")
	}
}
